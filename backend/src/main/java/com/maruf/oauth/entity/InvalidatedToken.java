 ...
 * the token expires.
 * This document is only checked by the auth service; the backend API has no
 * MongoDB access and
 * relies solely on JWT expiration for validation.
 *
 * @author Maruf Bepary
 * @see com.maruf.oauth.service.RefreshTokenStore for blacklist operations
 * @see com.maruf.oauth.config.JwtAuthenticationFilter for blacklist verification
 */
